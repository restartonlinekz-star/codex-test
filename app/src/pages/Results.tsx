...
import { Calendar, Clock, CheckCircle, TrendingUp, Award, Trash2, Eye, Loader2 } from 'lucide-react';
import { getTestResults, clearTestResults } from '@/services/api';
...
export function Results() {
  ...
  const loadResults = async () => {
    try {
      ...
      setError('Нәтижелерді жүктеу қатесі');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClearResults = async () => {
    if (confirm('Барлық нәтижелерді жоюға сенімдісіз бе?')) {
      const success = await clearTestResults();
      if (success) {
        setResults([]);
      }
    }
    ...
    return (
      <div className="min-h-screen pt-24 pb-16 px-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-muted-foreground">Нәтижелер жүктелуде...</p>
        </div>
      </div>
    );
  }
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        <div 
                          ...
                          style={{ 
                            borderColor: '#00ffe040',
                            backgroundColor: '#00ffe010'
                          }}
                        >
                          📝
                        </div>
